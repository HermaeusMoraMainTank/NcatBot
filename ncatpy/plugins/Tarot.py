...
# 塔罗牌逻辑处理类
class Tarot:
    tarot_libraries: List[str] = []
    current_library_index: int = 0  # 保存当前图库的索引
    ...
    def __init__(self):
        self.current_library_index = 0  # 初始化索引为0
        ...
        if message == "占卜":
            ...
            if random.random() < probability:
                ...
                if folder.exists() and folder.is_dir():
                    files = [f for f in folder.iterdir() if f.suffix.lower() in [".png", ".jpg"]]
                    if files:
                        image = random.choice(files)
                        await input.add_at(input.user_id).add_image(str(image.resolve())).reply()
                        return
            ...
            for card in random_tarots:
                ...
                log.info(f"Selected tarot card: {card.name}, orientation: {'正位' if i == 0 else '逆位'}")
                await input.add_at(input.user_id).add_text("\n").add_text(self.get_tarot_message(card, i)).add_image(
                    image_bytes).reply()

    def get_tarot_message(self, tarot: TarotCard, i: int) -> str:
        description = "正位\n" + tarot.positive if i == 0 else "逆位\n" + tarot.negative
        return TarotConstant.FORMAT.replace("%牌名%", tarot.name).replace("%描述%", description)

    def get_tarot_message_by_sender(self, tarot: TarotCard, i: int) -> str:
        description = "正位\n" + tarot.positive if i == 0 else "逆位\n" + tarot.negative
        return TarotConstant.FORMAT2.replace("%牌名%", tarot.name).replace("%描述%", description)
    ...
    def load_tarot_data(self) -> List[TarotCard]:
        ...
        try:
            with open("data/yml/tarot.yml", "r", encoding="utf-8") as f:
                ...
                for t in tarot_data:
                    tarot = TarotCard(t["name"], t["positive"], t["negative"], t["imageName"])
