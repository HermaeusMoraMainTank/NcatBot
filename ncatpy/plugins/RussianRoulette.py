...
class RussianRoulette:
    ...
    async def shoot(self, input: GroupMessage) -> bool:
        ...
        if trigger_position == self.CLIP_SIZE - 1:
            await input.add_text(f"{user_name}很清楚这是必死之局。").reply()

        # 检查是否炸膛
        if random.random() < self.MALFUNCTION_PROBABILITY:
            await input.add_text("左轮手枪突然炸膛了...\n" + bot_name + "换了一把新的手枪。").reply()
            await self.reload(group_id)  # 使用 await 调用异步方法
            return False
        ...
        if trigger_position == bullet_position:
            ...
            await input.add_text(
                f"{user_name}的目光逐渐变得呆滞，他向后摔倒在地，看上去像是从来没有活过似的。\n{bot_name}枪下不幸的冤魂已有 {self.kill_count} 条，但她仍然重新装上了子弹。").reply()
            await self.reload(group_id)  # 使用 await 调用异步方法
            ...
            if random_number <= 20:
                ...
                image_path = "data/image/RussianRoulette/开枪.jpg" if random_image_index == 0 else "data/image/RussianRoulette/开枪.gif"
                await input.add_image(image_path).add_text(f"{bot_name}打出了暴击！").reply()
                try:
                    await input.set_group_ban(group_id=group_id, user_id=input.user_id,
            ...
            try:
                await input.set_group_ban(group_id=group_id, user_id=input.user_id, duration=self.DAMAGE_TIME)
                return True
            except Exception as e:
                log.info(f"无法禁言 {input.sender.id} {input.sender.nickname}")
                log.info(str(e))
            ...
            await input.add_text(
                f"{user_name}侥幸活过了一轮，但他终究难逃死亡的结局，每个人都会死。\n{bot_name}的左轮手枪还剩 {remaining_bullets} 发。").reply()
            # 更新扳机位置
            self.trigger_position_map[group_id] = (trigger_position + 1) % self.CLIP_SIZE
            return False