...
class JRRP:
    ...
    async def handle_jrrp(self, input: GroupMessage):
        ...
        if input.raw_message in ["jrrp", "今日人品"]:
            ...
            message_digest.update(bytes([sender.get_id()]))
            ...
            # 根据人品值返回不同的消息
            if luck_value == 0:
                await input.reply(f"{sender.nickname} 的今日人品是：{luck_value}。怎，怎么会这样……")
            elif 0 < luck_value <= 20:
                await input.reply(f"{sender.nickname} 的今日人品是：{luck_value}。推荐闷头睡大觉。")
            elif 20 < luck_value <= 40:
                await input.reply(f"{sender.nickname} 的今日人品是：{luck_value}。也许今天适合摆烂。")
            elif 40 < luck_value <= 60 and luck_value != 42:
                await input.reply(f"{sender.nickname} 的今日人品是：{luck_value}。又是平凡的一天。")
            elif 60 < luck_value <= 80 and luck_value != 77:
                await input.reply(f"{sender.nickname} 的今日人品是：{luck_value}。太阳当空照，花儿对你笑。")
            elif 80 < luck_value < 100:
                await input.reply(f"{sender.nickname} 的今日人品是：{luck_value}。出门可能捡到{luck_value}块钱。")
            elif luck_value == 42:
                await input.reply(f"{sender.nickname} 的今日人品是：{luck_value}。感觉可以参透宇宙的真理。")
            elif luck_value == 77:
                await input.reply(f"{sender.nickname} 的今日人品是：{luck_value}。要不要去抽一发卡试试呢……")
            elif luck_value == 100:
                await input.reply(f"{sender.nickname} 的今日人品是：{luck_value}。买彩票可能会中大奖哦！")
