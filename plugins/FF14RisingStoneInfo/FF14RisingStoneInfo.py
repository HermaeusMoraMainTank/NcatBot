...
class FF14RisingStoneInfo(NcatBotPlugin):
    ...
    for job in job_list:
        try:
            ...
            job_icons[job] = PILImage.open(icon_path)
        except Exception as e:
            _log.warning(f"Error loading job icon {job}: {e}")

        # 加载生产采集职业图标
        for i in range(11):
            try:
                icon_path = f"data/image/ff14/icon/sjob{i}.png"
                job_icons[f"sjob{i}"] = PILImage.open(icon_path)
            except Exception as e:
                _log.warning(f"Error loading sjob icon {i}: {e}")
    ...
    def read_cookie_from_file(self) -> str:
        ...
        try:
            ...
            _log.warning(f"Error reading cookie file: {e}")
            return ""
    ...
    def search_player(self, character_name: str, cookie: str) -> List[Dict]:
        ...
        try:
            ...
            if result.get("code") == 10000:
                ...
                _log.warning(f"API返回错误：{result.get('msg')}")
                return []
        except Exception as e:
            _log.warning("=== Error Info ===")
            _log.warning(f"Error Type: {type(e).__name__}")
            _log.warning(f"Error Message: {str(e)}")
            if hasattr(e, "response"):
                _log.warning(f"Response Status: {e.response.status_code}")
                _log.warning("Response Headers: %s", dict(e.response.headers.items()))
                _log.warning(f"Response Text: {e.response.text}")
            return []
    ...
    def generate_image(self, user_data: Dict) -> str:
        ...
        if avatar_url:
            try:
                avatar_image = PILImage.open(BytesIO(httpx.get(avatar_url).content))
    ...
    def draw_gradient_background(self, image: PILImage.Image, width: int, height: int):
        """绘制渐变背景"""
