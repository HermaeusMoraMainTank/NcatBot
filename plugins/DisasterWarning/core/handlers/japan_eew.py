...
class JMAEEWFanStudioHandler(BaseDataHandler):
    ...
    def _parse_data(self, data: dict[str, Any]) -> DisasterEvent | None:
        ...
        try:
            ...
            if not msg_data:
                _log.warning(f"[灾害预警] {self.source_id} 消息中没有有效数据")
                return None

            # 记录数据获取情况用于调试
            if "Data" in data:
                _log.debug(f"[灾害预警] {self.source_id} 使用Data字段获取数据")
            elif "data" in data:
                _log.debug(f"[灾害预警] {self.source_id} 使用data字段获取数据")
            else:
                _log.debug(f"[灾害预警] {self.source_id} 使用整个消息作为数据")

            # 检查是否为地震预警数据 - JMA数据也有epiIntensity字段
            if "epiIntensity" not in msg_data and "infoTypeName" not in msg_data:
                _log.debug(f"[灾害预警] {self.source_id} 非JMA地震预警数据，跳过")
                return None

            # 检查是否为取消报
            if msg_data.get("cancel", False):
                _log.info(f"[灾害预警] {self.source_id} 收到取消报，跳过")
                return None
    ...
    def parse_message(self, message: str) -> DisasterEvent | None:
        ...
        try:
            ...
            if code == 556:  # 緊急地震速報（警報）
                _log.debug(f"[灾害预警] {self.source_id} 收到緊急地震速報（警報）")
                ...
                _log.debug(f"[灾害预警] {self.source_id} 非EEW数据，code: {code}")
                return None

        except json.JSONDecodeError as e:
            _log.error(f"[灾害预警] {self.source_id} JSON解析失败: {e}")
            return None
        except Exception as e:
            _log.error(f"[灾害预警] {self.source_id} 消息处理失败: {e}")
            return None
    ...
    def _parse_eew_data(self, data: dict[str, Any]) -> DisasterEvent | None:
        ...
        try:
            ...
            if "time" in earthquake_info:
                ...
                _log.warning(f"[灾害预警] {self.source_id} 缺少地震时间信息")
            ...
            if is_cancelled:
                _log.info(f"[灾害预警] {self.source_id} 收到取消的EEW事件")

            # 检查test字段
            is_test = data.get("test", False)
            if is_test:
                _log.info(f"[灾害预警] {self.source_id} 收到测试模式的EEW事件")
            ...
            _log.error(f"[灾害预警] {self.source_id} 解析EEW数据失败: {e}")
            return None
    ...
    def _parse_data(self, data: dict[str, Any]) -> DisasterEvent | None:
        ...
        try:
            ...
            if data.get("type") != "jma_eew":
                _log.debug(f"[灾害预警] {self.source_id} 非JMA EEW数据，跳过")
                return None
