...
(使用 aiohttp 实现以获得更好的跨平台兼容性) - NcatBot 版本
"""
...
class WebSocketManager:
    ...
    def __init__(self, config: dict[str, Any], message_logger=None):
        ...
        self.fallback_retry_counts: dict[str, int] = {}
    ...
    def register_handler(self, connection_name: str, handler: Callable):
        ...
        _log.info(f"[灾害预警] 注册处理器: {connection_name}")
    ...
    async def connect(
        ...
        try:
            ...
            connect_kwargs = {
                ...
                "max_msg_size": self.config.get("max_message_size", 2**20),
            }
            ...
            async with self.session.ws_connect(**connect_kwargs) as websocket:
                ...
                _log.info(f"[灾害预警] WebSocket连接成功: {name}")
                self.connection_retry_counts[name] = 0
                    ...
                    async for msg in websocket:
                        if msg.type == WSMsgType.TEXT:
                            ...
                            try:
                                ...
                                _log.error(f"[灾害预警] 消息处理错误 {name}: {e}")
                                _log.debug(
            ...
            _log.warning(f"[灾害预警] 连接中断或失败 {name}: {e}")
    ...
    async def disconnect(self, name: str):
        ...
        if name in self.connections:
            try:
                ...
                _log.info(f"[灾害预警] WebSocket连接已关闭: {name}")
            except Exception as e:
                _log.error(f"[灾害预警] WebSocket断开连接时出错 {name}: {e}")
            finally:
                self.connections.pop(name, None)
    ...
    async def send_message(self, name: str, message: str):
        ...
        if name in self.connections:
            try:
                ...
                _log.debug(f"[灾害预警] 消息已发送到 {name}: {message[:100]}...")
            except Exception as e:
                _log.error(f"[灾害预警] WebSocket管理器发送消息失败 {name}: {e}")
        else:
            _log.warning(f"[灾害预警] WebSocket管理器尝试发送到未连接的连接: {name}")
    ...
    async def start(self):
        ...
        if not self.session or self.session.closed:
            ...
            _log.info("[灾害预警] WebSocket管理器已启动")

        if not self.message_handlers:
            _log.warning("[灾害预警] 没有注册任何消息处理器")

    async def stop(self):
        """停止管理器"""
        _log.info("[灾害预警] WebSocket管理器正在停止...")
        self.running = False
        ...
        _log.info("[灾害预警] WebSocket管理器已停止")
        ...
        try:
            async with self.session.get(url, headers=headers) as response:
                if response.status == 200:
                    ...
                    _log.warning(f"[灾害预警] HTTP请求失败 {url}: {response.status}")
        except Exception as e:
