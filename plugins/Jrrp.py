        ...
        if input.raw_message in ["jrrp", "今日人品"]:
            ...
            if sender.user_id in [HMMT.HMMT_ID, 1042081663, 864772045]:
                await input.add_text(
                    f"{sender.nickname} 的每日人品都会是：101。"
                ).reply()
                return
            # 计算今日人品值
            ...
            _log.info(f"Calculated luck value: {luck_value}")

            # 根据人品值返回不同的消息
            if luck_value == 0:
                await input.add_text(
                    f"{sender.nickname} 的今日人品是：{luck_value}。怎，怎么会这样……"
                ).reply()
            elif 0 < luck_value <= 20:
                await input.add_text(
                    f"{sender.nickname} 的今日人品是：{luck_value}。推荐闷头睡大觉。"
                ).reply()
            elif 20 < luck_value <= 40:
                await input.add_text(
                    f"{sender.nickname} 的今日人品是：{luck_value}。也许今天适合摆烂。"
                ).reply()
            elif 40 < luck_value <= 60 and luck_value != 42:
                await input.add_text(
                    f"{sender.nickname} 的今日人品是：{luck_value}。又是平凡的一天。"
                ).reply()
            elif 60 < luck_value <= 80 and luck_value != 77:
                await input.add_text(
                    f"{sender.nickname} 的今日人品是：{luck_value}。太阳当空照，花儿对你笑。"
                ).reply()
            elif 80 < luck_value < 100:
                await input.add_text(
                    f"{sender.nickname} 的今日人品是：{luck_value}。出门可能捡到{luck_value}块钱。"
                ).reply()
            elif luck_value == 42:
                await input.add_text(
                    f"{sender.nickname} 的今日人品是：{luck_value}。感觉可以参透宇宙的真理。"
                ).reply()
            elif luck_value == 77:
                await input.add_text(
                    f"{sender.nickname} 的今日人品是：{luck_value}。要不要去抽一发卡试试呢……"
                ).reply()
            elif luck_value == 100:
                await input.add_text(
                    f"{sender.nickname} 的今日人品是：{luck_value}。买彩票可能会中大奖哦！"
                ).reply()
