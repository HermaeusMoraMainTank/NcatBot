...
from NcatBot.common.entity.GroupMember import GroupMember
...
class TodayWaifu(BasePlugin):
    ...
    async def get_random_wife(self, input: GroupMessage, group_id: int):
        ...
        if (
            ...
            return await input.get_group_member_info(
        ...
        if not filtered_members:
            bot_member_info = await input.get_group_member_info(
                group_id=group_id, user_id=HMMT.BOT_ID
            )
    ...
    async def handle_message(self, input: GroupMessage):
        ...
        if message == "今日老婆":
            ...
            if user_id in user_to_wife_map:
                ...
                wife_info = await input.get_group_member_info(
                ...
                if isinstance(wife_info, dict) and wife_info.get("status") == "ok":
                    ...
                    if wife_data:  # 确保 data 字段存在且不为空
                        ...
                        return await (
                            ...
                            .add_text(f"你今天的群友老婆是：")
                            .add_image(avatar_url)
                            .add_text(f" {wife_info.nickname}({wife_info.user_id})")
                        ).reply()
                else:
            ...
            return await (
                ...
                .add_text(f"你今天的群友老婆是：")
                .add_image(avatar_url)
                .add_text(f" {new_wife.nickname}({new_wife.user_id})")
            ).reply()
        ...
        if (
            ...
            await input.add_text(
                f"@{input.sender.nickname} 成功更换了 {target_username} 的老婆。"
            ).reply()
            return (
                await input.add_at(target_user_id)
                .add_text(f"你的老婆被 {input.sender.nickname} 更换了。")
                .reply()
            )
