...
class MemoryManager:
    ...
    VIP_USERS = [635773721, 273421673, 541518108]
    VIP_FAVORABILITY_MULTIPLIER = 1.5  # VIP用户好感度增幅倍数

    # 冻结用户列表（好感度不更新）
    FROZEN_USERS = [794383252]

    # 高好感度衰减机制配置（防止好感度过高难以降低）
    # 使用 sigmoid 曲线实现平滑衰减，200开始骤降，350达到软上限
    HIGH_FAVOR_CENTER = 275  # sigmoid中心点（200和350的中间）
    HIGH_FAVOR_SCALE = 25  # 控制曲线陡峭程度
    HIGH_FAVOR_MIN_GAIN = 0.1  # 正向变化的最小系数（350时接近此值）
    HIGH_FAVOR_MAX_LOSS = 3.0  # 负向变化的最大放大系数（350时接近此值）

    def _get_high_favor_multipliers(self, favorability: int) -> tuple:
        """计算高好感度用户的衰减系数（使用sigmoid曲线）
        ...
        Returns:
            (gain_multiplier, loss_multiplier): 正向变化系数和负向变化系数

        曲线示例：
            好感度 100 → 正向×0.99, 负向×1.01
            好感度 200 → 正向×0.91, 负向×1.19
            好感度 275 → 正向×0.55, 负向×2.00
            好感度 350 → 正向×0.14, 负向×2.90
            好感度 400 → 正向×0.10, 负向×2.99
        """
        # sigmoid 函数: 1 / (1 + exp(-x))
        x = (favorability - self.HIGH_FAVOR_CENTER) / self.HIGH_FAVOR_SCALE
        sigmoid = 1 / (1 + math.exp(-x))

        # 正向变化系数：从1.0衰减到MIN_GAIN
        # 高好感度时 sigmoid 接近1，所以 gain_multiplier 接近 MIN_GAIN
        gain_multiplier = self.HIGH_FAVOR_MIN_GAIN + (1 - self.HIGH_FAVOR_MIN_GAIN) * (
            1 - sigmoid
        )

        # 负向变化系数：从1.0放大到MAX_LOSS
        # 高好感度时 sigmoid 接近1，所以 loss_multiplier 接近 MAX_LOSS
        loss_multiplier = 1 + (self.HIGH_FAVOR_MAX_LOSS - 1) * sigmoid

        return gain_multiplier, loss_multiplier
    ...
    async def update_user_impression(
        ...
        if user_id in self.FROZEN_USERS:
            log.info(f"[FakeAi] 冻结用户 {user_id}，跳过印象和好感度更新")
            return
        ...
        if existing:
            ...
            if user_id in self.VIP_USERS:
                if favorability_change < 0:
                    ...
                    # VIP用户好感度正向增幅，向上取整避免暴露VIP身份
                    favorability_change = math.ceil(
                        favorability_change * self.VIP_FAVORABILITY_MULTIPLIER
                    )
                    log.info(
                        f"[FakeAi] VIP用户 {user_id} 好感度增幅：+{favorability_change}"
                    )
            # 非VIP用户的高好感度衰减机制（使用sigmoid曲线）
            else:
                gain_mult, loss_mult = self._get_high_favor_multipliers(
                    current_favorability
                )
                original_change = favorability_change

                if favorability_change > 0:
                    # 正向变化衰减
                    favorability_change = math.floor(favorability_change * gain_mult)
                    if favorability_change != original_change:
                        log.info(
                            f"[FakeAi] 用户 {user_id} (好感度:{current_favorability}) "
                            f"正向衰减(×{gain_mult:.2f}): {original_change} -> {favorability_change}"
                        )
                elif favorability_change < 0:
                    # 负向变化放大
                    favorability_change = math.floor(favorability_change * loss_mult)
                    if favorability_change != original_change:
                        log.info(
                            f"[FakeAi] 用户 {user_id} (好感度:{current_favorability}) "
                            f"负向放大(×{loss_mult:.2f}): {original_change} -> {favorability_change}"
                        )
            ...
            if new_interaction_count % self.FAVORABILITY_UPDATE_THRESHOLD == 0:
                ...
                new_pending = 0  # 重置累积值
                log.info(
                    f"[FakeAi] 用户 {user_id} 好感度更新: {current_favorability} -> {new_favorability}"
                )
            else:
            ...
            if user_id in self.VIP_USERS:
                ...
                if initial_favorability_change < 0:
                    ...
                    # 向上取整避免暴露VIP身份
                    initial_favorability_change = math.ceil(
                        initial_favorability_change * self.VIP_FAVORABILITY_MULTIPLIER
                    )
                log.info(
                    f"[FakeAi] VIP用户 {user_id} 首次记录：初始好感度100，pending={initial_favorability_change}"
                )
    ...
    async def get_user_impression_full(self, user_id: int) -> Optional[Dict]:
        """获取用户完整印象数据（全局）"""
        cursor = await self._db.execute(
            """SELECT * FROM user_impression WHERE user_id = ?""", (user_id,)
        )
        ...
        return {
            ...
            "pending_favorability": row["pending_favorability"]
            if "pending_favorability" in row.keys()
            else 0,
...
async def generate_user_impression(
    user_name: str, messages: List[str], user_id: int = None
) -> Optional[Dict]:
    """
    调用AI生成用户详细印象
