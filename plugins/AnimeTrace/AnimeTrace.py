...
from ncatbot.core import GroupMessage, MessageChain, Text, Image as BotImage, Reply
...
class AnimeTracePlugin(NcatBotPlugin):
    ...
    async def _get_image_from_message(
        ...
        """
        # 先从当前消息中查找图片
        images = message.message.filter(BotImage)
        if images:
        ...
        if reply_list:
            ...
            # 从回复消息中获取图片
            reply_images = reply_msg.message.filter(BotImage)
            if reply_images:
    ...
    def _convert_image_to_jpeg(self, img: Image.Image) -> bytes:
        """将图片转换为JPEG格式的字节流"""
        # 如果图片是RGBA模式，转换为RGB
        if img.mode == "RGBA":
            # 创建白色背景
    ...
    def _search_character(
        self, base_img: Image.Image, model: str, ai_detect: int = 0
    ) -> AnimeTraceResult:
        """调用API识别角色"""
        # 将图片转换为base64
    ...
    def _construct_result_message(
        self, base_img: Image.Image, result: AnimeTraceResult, mode: str
    ) -> MessageChain:
        """构造结果消息"""
        elements = []
        ...
        if char_nums == 0:
            elements.append(Text("没有识别到任何角色"))
        ...
        if result.ai:
            start_msg = "该图可能是ai绘图!\n" + start_msg
        elements.append(Text(start_msg))
        ...
        for box_item in result.data:
            ...
            for i, char in enumerate(characters, 1):
                ...
                elements.append(Text(f"角色:{name}\n"))
                elements.append(Text(f"来自{mode}:{char.work}\n"))

                # 添加萌娘百科链接（可选）
                moegirl_link = f"萌娘百科:zh.moegirl.org.cn/index.php?search={q}\n"
                elements.append(Text(moegirl_link))

                # 添加维基百科链接
                wiki_link = f"zh.wikipedia.org/w/index.php?search={q}\n"
                elements.append(Text(wiki_link))
            ...
            elements.append(BotImage(temp_file))
