...
class TodayWaifu(NcatBotPlugin):
    ...
    async def get_random_wife(self, input: GroupMessage, group_id: str):
        ...
        # 检查响应是否成功
        if not hasattr(members_response, "members") or not members_response.members:
    ...
    async def handle_message(self, input: GroupMessage):
        ...
        if message_text == "今日老婆":
            ...
            if SpecialGroupHandler.should_use_special_logic(group_id, user_id):
                ...
                if special_partner_id:
                    ...
                    partner_info = await self.api.get_group_member_info(
                        group_id=group_id, user_id=special_partner_id
                    )
                    # 检查响应类型并解析
                    if hasattr(partner_info, "user_id"):  # 如果是 GroupMemberInfo 对象
                        # 将 GroupMemberInfo 对象转换为字典
                        partner_data = {
                            "group_id": partner_info.group_id,
                            "user_id": partner_info.user_id,
                            "nickname": partner_info.nickname,
                            "card": partner_info.card,
                            "role": partner_info.role,
                            "title": partner_info.title,
                            "level": partner_info.level,
                            "sex": partner_info.sex,
                            "age": partner_info.age,
                            "area": partner_info.area,
                            "qq_level": partner_info.qq_level,
                            "join_time": partner_info.join_time,
                            "last_sent_time": partner_info.last_sent_time,
                            "title_expire_time": partner_info.title_expire_time,
                            "unfriendly": partner_info.unfriendly,
                            "card_changeable": partner_info.card_changeable,
                            "is_robot": partner_info.is_robot,
                            "shut_up_timestamp": partner_info.shut_up_timestamp,
                        }
            ...
            return await self.api.post_group_msg(group_id=input.group_id, rtf=message)

        if (
            input.raw_message.startswith("换")
            and "的老婆" in input.raw_message
            and (user_id == str(HMMT.HMMT_ID) or user_id == "3860435136")
        ):
            target_user_id = None
            new_wife_id = None
            at_count = 0
            ...
            for element in input.message:
                if hasattr(element, "msg_seg_type") and element.msg_seg_type == "at":
                    if at_count == 0:
                        target_user_id = str(element.qq)
                    elif at_count == 1:
                        new_wife_id = str(element.qq)
                    at_count += 1
            ...
            await self.api.post_group_msg(group_id=input.group_id, rtf=message1)

            message2 = MessageArray()
            message2.add_at(target_user_id)
            message2.add_text(f" 你的老婆被 {input.sender.nickname} 更换了。")
            return await self.api.post_group_msg(group_id=input.group_id, rtf=message2)
