...
from ncatbot.core.message import At, GroupMessage, Image, MessageChain, Text
    ...
    async def handle_message(self, input: GroupMessage):
        ...
        if message == "今日老婆":
            ...
            if 0.05 <= random_value <= 0.15:
                return await self.api.post_group_msg(group_id=input.group_id, text="今*老婆").reply()
            if random_value < 0.05:
                return await self.api.post_group_msg(group_id=input.group_id, text="你快醒醒 你没有老婆").reply()
            ...
            if user_id in user_to_wife_map:
                ...
                if isinstance(wife_info, dict) and wife_info.get("status") == "ok":
                    ...
                    return await self.api.post_group_msg(group_id=input.group_id, text="获取老婆信息失败，请稍后再试。").reply()
        ...
        if (
            ...
            if target_user_id not in user_to_wife_map:
                return await self.api.post_group_msg(group_id=input.group_id, text=f"{target_username} 没有老婆，无法更换。").reply()
            ...
            await self.api.post_group_msg(group_id=input.group_id, text=f"@{input.sender.nickname} 成功更换了 {target_username} 的老婆。").reply()
            return (
        ...
        if message == "换一个老婆" and user_id == HMMT.HMMT_ID:
            ...
            if user_id not in user_to_wife_map:
                return await self.api.post_group_msg(group_id=input.group_id, text="你还没有老婆，无法换一个老婆。").reply()
            ...
            new_wife = await self.get_random_wife(input, group_id)
            if new_wife is None:
                return await self.api.post_group_msg(group_id=input.group_id, text="无法获取新的老婆，请稍后再试。").reply()
            ...
            return await self.api.post_group_msg(group_id=input.group_id, rtf=MessageChain(
                [
                    At(user_id),
                    Text(f"@{input.sender.nickname} 成功更换了老婆，你的新老婆是：{new_wife.nickname}"),
                ]
            ))
