...
class SpecialGroupHandler:
    ...
    @classmethod
    def is_special_group(cls, group_id: int) -> bool:
        """判断是否为特殊群组"""
        return group_id == cls.SPECIAL_GROUP_ID

    @classmethod
    def is_special_user(cls, user_id: int) -> bool:
        """判断是否为特殊用户"""
        return user_id in cls.SPECIAL_USER_IDS

    @classmethod
    def get_special_partner(cls, user_id: int) -> Optional[int]:
        """获取特殊用户的默认伴侣"""
        if not cls.is_special_user(user_id):
            return None

        # 3860435136 的伴侣是 273421673
        if user_id == 3860435136:
            return 273421673
        # 273421673 的伴侣是 3860435136
        elif user_id == 273421673:
            return 3860435136
    ...
    def filter_special_members(
        cls, members: List[GroupMember], current_user_id: int
    ) -> List[GroupMember]:
        ...
        for member in members:
            # 如果是特殊用户且不是当前用户，则跳过
            if (
                cls.is_special_user(member.user_id)
    ...
    @classmethod
    def should_use_special_logic(cls, group_id: int, user_id: int) -> bool:
        """判断是否应该使用特殊逻辑"""
