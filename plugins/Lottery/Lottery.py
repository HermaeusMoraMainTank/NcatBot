...
class Lottery(NcatBotPlugin):
    ...
    async def on_load(self):
        ...
        _log.debug(f"Initial total_awards after loading: {self.total_awards}")
            ...
            if os.path.exists(self.FILE_PATH):
                with open(self.FILE_PATH, "r", encoding="utf-8") as file:
                    ...
                    if line.strip():
                        ...
                        _log.info(f"成功加载大乐透奖品总数: {self.total_awards}")
                        _log.debug(
                            f"Initial total_awards after loading: {self.total_awards}"
                        )
                    else:
                        self.total_awards = 0
                        _log.info("奖品总数文件为空，初始化为 0")
            else:
                self.total_awards = 0
                _log.info("奖品总数文件不存在，初始化为 0")
        except (IOError, ValueError) as e:
            self.total_awards = 0
            _log.warning(f"无法加载奖品总数，初始化为 0: {e}")
    ...
    def save_total_awards(self):
        ...
        try:
            ...
            _log.debug(f"total_awards before saving: {self.total_awards}")
            _log.debug(f"total_awards after saving: {self.total_awards}")
        except IOError as e:
            _log.error(f"无法保存奖品总数: {e}")
    ...
    async def handle_punish_all(self, input: GroupMessage, participants: List[int]):
        ...
        await self.api.qq.post_group_msg(group_id=input.group_id, text="恭喜这位……")
