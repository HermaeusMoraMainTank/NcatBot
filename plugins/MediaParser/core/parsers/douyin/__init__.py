...
class DouyinParser(BaseParser):
    ...
    def _load_cookies(self):
        ...
        try:
            ...
            if self.douyin_ck:
                self._set_cookies(self.douyin_ck)
                _log.info(f"已从 {self._cookies_file} 加载抖音 cookies")
        except Exception as e:
            _log.warning(f"加载抖音 cookies 失败: {e}")
    ...
    def _update_cookies_from_response(self, set_cookie_headers: list[str]):
        ...
        if self.douyin_ck:
            ...
            _log.debug(f"[抖音] 现有 cookies 数量: {len(existing_cookies)}")
        ...
        _log.debug(f"[抖音] 新增/更新的 cookies: {new_cookie_names}")
    ...
    async def _parse_douyin(self, searched: re.Match[str]):
        ...
        _log.debug(f"[抖音] 解析类型: {ty}, ID: {vid}")
        if ty == "slides":
        ...
        _log.debug(f"[抖音] 尝试解析URL列表: {urls}")

        for url in urls:
            try:
                _log.debug(f"[抖音] 尝试解析: {url}")
                return await self.parse_video(url)
            except ParseException as e:
                _log.warning(f"[抖音] 解析失败 {url}, 错误: {e}")
                continue
    ...
    async def parse_with_redirect(
        ...
        async with self.client.get(
            ...
            _log.debug(f"[抖音] 短链重定向响应状态码: {resp.status}")
            # 从响应中提取 Set-Cookie 并更新
            set_cookie_headers = resp.headers.getall("Set-Cookie", [])
            if set_cookie_headers:
                _log.debug(f"[抖音] 收到 {len(set_cookie_headers)} 个 Set-Cookie")
            ...
            if resp.status in (301, 302, 303, 307, 308):
                ...
                _log.debug(f"[抖音] 重定向到: {redirect_url}")
    ...
    async def parse_video(self, url: str):
        ...
        async with self.client.get(
            ...
            _log.debug(f"[抖音] 视频页面响应状态码: {resp.status}")
            if resp.status != 200:
                raise ParseException(f"status: {resp.status}")
            text = await resp.text()
            _log.debug(f"[抖音] 响应体大小: {len(text)} 字符")
            # 从响应中提取 Set-Cookie 并更新
            set_cookie_headers = resp.headers.getall("Set-Cookie", [])
            if set_cookie_headers:
                _log.debug(f"[抖音] 收到 {len(set_cookie_headers)} 个 Set-Cookie")
        ...
        if not matched or not matched.group(1):
            _log.debug("[抖音] 未在HTML中找到 window._ROUTER_DATA")
            raise ParseException("can't find _ROUTER_DATA in html")

        _log.debug("[抖音] 成功提取 window._ROUTER_DATA")
    ...
    async def parse_slides(self, video_id: str):
        ...
        async with self.client.get(
            ...
            _log.debug(f"[抖音] 幻灯片API响应状态码: {resp.status}")
