...
class BilibiliParser(BaseParser):
    ...
    @handle("mall.bili", r"mall\.bilibili\.com/[A-Za-z\d\._?%&+\-=/#]+itemsId=(?P<item_id>\d+)[A-Za-z\d\._?%&+\-=/#]*")
    async def _parse_mall_item(self, searched: Match[str]):
        ...
        return self.result(
            title=title.strip() if isinstance(title, str) else f"B站会员购商品 {item_id}",
    ...
    async def _parse_read_fallback(self, read_id: int):
        ...
        if result.get("code") != 0:
            raise ParseException(f"获取文章信息失败: {result.get('message', '未知错误')}")
        ...
        if stats:
            ...
            desc_parts.append(f"阅读: {view} | 点赞: {like} | 投币: {coin} | 收藏: {favorite}")
    ...
    async def extract_download_urls(
        ...
        *,
        ...
        _log.debug(f"视频流质量: {video_stream.video_quality.name}, 编码: {video_stream.video_codecs}")
