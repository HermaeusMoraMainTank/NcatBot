...
class YouTubeParser(BaseParser):
    ...
    def _set_cookies(self):
        raw = (self.config.get("ytb_ck") or "").strip()
        if raw:
            cookie_path = Path(raw)
            if cookie_path.is_file():
                self.config["ytb_cookies_file"] = str(cookie_path)
                return
            ytb_cookies_file = self.data_dir / "ytb_cookies.txt"
            ytb_cookies_file.parent.mkdir(parents=True, exist_ok=True)
            if raw.startswith("#") or "\t" in raw:
                ytb_cookies_file.write_text(raw, encoding="utf-8")
            else:
                save_cookies_with_netscape(raw, ytb_cookies_file, "youtube.com")
