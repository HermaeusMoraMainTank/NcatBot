...
class NGAParser(BaseParser):
    ...
    @staticmethod
    async def _read_response_text(resp) -> str:
        """优先按 UTF-8 解码，失败时回退常见中文编码，避免站点声明编码不准确导致崩溃。"""
    ...
    async def _parse(self, searched: re.Match[str]):
        ...
        if content_tag and isinstance(content_tag, Tag):
            ...
            if img_urls:
                ...
                if guest_js:
                    image_headers["Cookie"] = f"guestJs={guest_js}"

                # NGA 图床易触发并发限频（HTTP 567），改为串行下载并加轻微间隔
                failed_count = 0
                failed_urls: list[str] = []
                for idx, image_url in enumerate(img_urls):
                    try:
                        image_path = await self.downloader.download_img(
                            image_url,
                            ext_headers=image_headers,
                            proxy=self.proxy,
                        )
                        contents.append(ImageContent(image_path))
                    except DownloadException:
                        failed_count += 1
                        failed_urls.append(image_url)
                        _log.warning(f"NGA 图片下载失败，跳过: {image_url}")
                    if idx < len(img_urls) - 1:
                        await asyncio.sleep(0.35)

                if failed_count > 0:
                    warn_text = f"{failed_count} 张图片下载失败，可能是图床风控限频"
                    text = f"{text}\n\n（{warn_text}）" if text else warn_text
        else:
            failed_urls = []
        ...
        return self.result(
            ...
            contents=contents,
            timestamp=timestamp,
            extra={"fallback_image_urls": failed_urls} if failed_urls else {},
        )
