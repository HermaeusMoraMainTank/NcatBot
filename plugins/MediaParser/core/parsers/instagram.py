    ...
    def _select_media_urls(
        ...
        if isinstance(formats, list) and formats:
            ...
            if video_fmt and not audio_fmt:
                _log.warning("Instagram audio format not found, fallback to combined")
            combined_fmt = self._best_av_format(formats)
            if combined_fmt:
                _log.warning("Instagram using combined format for download")
                return combined_fmt["url"], None

        direct_url = self._entry_video_url(info)
        if direct_url:
            _log.warning("Instagram formats missing, using direct URL download")
