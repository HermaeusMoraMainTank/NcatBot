...
class Downloader:
    ...
    def __init__(self, config: AstrBotConfig):
        ...
        self._download_timeout = config["download_timeout"]

    @property
    def client(self) -> ClientSession:
        """惰性创建 ClientSession，确保在正确的事件循环中"""
        if self._client is None or self._client.closed:
            self._client = ClientSession(
                timeout=ClientTimeout(total=self._download_timeout)
            )
        return self._client
        ...
        for attempt in range(retries + 1):
            try:
                async with self.client.get(
                    ...
                    if content_length == 0:
                        _log.warning(f"媒体 url: {url}, 大小为 0, 取消下载")
                    ...
                    if downloaded == 0:
                        _log.warning(f"媒体 url: {url}, 实际大小为 0, 取消下载")
                        raise ZeroSizeException
                ...
                if attempt < retries:
                    ...
                    continue
                _log.exception(f"下载失败 | url: {url}, file_path: {file_path}")
    ...
    def _build_ytdlp_opts(
        self, cookiefile: Path | None = None, **extra: Any
    ) -> dict[str, Any]:
        opts: dict[str, Any] = {
            "quiet": True,
            "noprogress": True,
        ...
        browser = (self.config.get("ytb_cookies_from_browser") or "").strip()
        if browser:
            opts["cookiesfrombrowser"] = (browser,)
        elif cookiefile and cookiefile.is_file():
    ...
    async def ytdlp_extract_info(
        ...
        opts = self._build_ytdlp_opts(
            cookiefile, skip_download=True, force_generic_extractor=False
        )
