...
def suppress_exception(
    ...
    def wrapper(*args: P.args, **kwargs: P.kwargs) -> T | None:
        try:
            ...
            _log.debug(f"函数 {func.__name__} 执行失败: {e}")
            return None
...
class Renderer:
    ...
    def _load_fonts(cls):
        ...
        _log.debug(f"加载字体「{font_path.name}」成功")

    @classmethod
    def _load_video_button(cls):
        """预加载视频按钮"""
        # convert("RGBA") 在已是 RGBA 时会返回原图自身；必须在关闭文件前 copy，否则 with 退出后 _im 失效
        with Image.open(cls.DEFAULT_VIDEO_BUTTON_PATH) as img:
            cls.video_button_image: PILImage = img.convert("RGBA").copy()
        ...
        for p in cls.LOGOS_DIR.rglob("*.png"):
            try:
                with Image.open(p) as img:
                    cls.platform_logos[p.stem] = img.convert("RGBA").copy()
            except Exception:
                continue
