...
from ncatbot.core import MessageChain, Text, Image as NcatImage, GroupMessage
...
class Box(NcatBotPlugin):
    ...
    def _get_target_ids(self, input: GroupMessage, command: str) -> List[int]:
        ...
        for seg in input.message:
            if hasattr(seg, "msg_seg_type") and seg.msg_seg_type == "at":
                target_id = int(seg.qq)
                # 过滤掉Bot自己和保护名单中的用户
    ...
    async def _box_user(self, input: GroupMessage, target_id: int, group_id: int):
        ...
        try:
            # get_stranger_info 直接返回 dict
            stranger_info = await self.api.get_stranger_info(user_id=target_id)
            # 确保返回的是字典
            if not stranger_info or not isinstance(stranger_info, dict):
                stranger_info = {}
            log.debug(f"获取到的用户信息: {stranger_info}")
        except Exception as e:
            log.warning(f"获取用户信息失败: {e}")
            await self.api.post_group_msg(
                group_id=group_id, rtf=MessageChain([Text("无效QQ号")])
            )
            return

        try:
            # get_group_member_info 返回 GroupMemberInfo 对象，需要转换为 dict
            result = await self.api.get_group_member_info(
                group_id=group_id, user_id=target_id
            )
            if result:
                member_info = asdict(result)
        except Exception as e:
        ...
        if not display:
            await self.api.post_group_msg(
                group_id=group_id,
                rtf=MessageChain([Text("获取用户信息失败或信息为空")]),
            )
            return
        ...
        if cache_path.exists():
            ...
            log.debug(f"命中缓存: {cache_path}")
        else:
            # 生成卡片图片
            if not self.renderer:
                await self.api.post_group_msg(
                    group_id=group_id, rtf=MessageChain([Text("卡片生成器未初始化")])
                )
                return
            ...
            try:
                ...
                log.debug(f"写入缓存: {cache_path}")
            except Exception as e:
                log.error(f"生成卡片失败: {e}")
                await self.api.post_group_msg(
                    group_id=group_id, rtf=MessageChain([Text(f"生成卡片失败: {e}")])
                )
                return
        ...
        # 发送消息
        message = MessageChain([NcatImage(str(temp_path))])

        if self.recall_time > 0:
            # 发送并设置撤回
            result = await self.api.post_group_msg(group_id=group_id, rtf=message)
    ...
    async def _recall_message(self, message_id: int, delay: int):
        ...
        try:
            await self.api.delete_msg(message_id=message_id)
            log.info(f"已自动撤回消息: {message_id}")
        except Exception as e:
            log.error(f"撤回消息失败: {e}")
