...
class Universalis(NcatBotPlugin):
    ...
    def setup(self):
        ...
        try:
            with open(
                os.path.join(os.getcwd(), "data/json/Item.json"), "r", encoding="utf-8"
            ) as f:
                data = json.load(f)
                self.item = Item(data)
        ...
        # 支持的大区列表
        self.supported_dc = ["陆行鸟", "莫古力", "猫小胖", "豆豆柴"]
        ...
        if len(key) == 3 and key[0] == "搜索物品":
            ...
            if server_name in self.supported_dc:
                ...
                await self.api.qq.post_group_msg(
                    group_id=input.group_id, text=f"未知的服务器或大区: {server_alias}"
                )
                return
            ...
            if item_id <= 0:
                ...
                if item_id <= 0:
                    await self.api.qq.post_group_msg(
                        group_id=input.group_id, text=f"未找到物品: {item_name}"
                    )
                    return
            ...
            if not data:
                await self.api.qq.post_group_msg(
                    group_id=input.group_id, text="没有找到数据喵"
                )
                return
            if not data.get("listings"):
                await self.api.qq.post_group_msg(
                    group_id=input.group_id, text="没有数据喵"
                )
                return
    ...
    def get_item_id(self, item_name: str) -> int:
        ...
        if item_name in self.item.Data["data"]:
            _log.debug(f"使用本地物品 ID 搜索: {item_name}")
            return self.item.Data["data"][item_name]
        else:
            _log.debug(f"未找到物品: {item_name}")
            return 0
                ...
                if results:
                    ...
                    best_match = max(
                        ...
                        key=lambda x: SequenceMatcher(
                            None, x["Name"], item_name
                        ).ratio(),
                    )
                    item_name = best_match["Name"]
                    item_id = best_match["ID"]
                    _log.debug(f"模糊搜索找到物品: {item_name} (ID: {item_id})")
                    return item_name, item_id
        except Exception as e:
            _log.error(f"模糊搜索时发生错误: {e}")
    ...
    def get_data(self, server_name: str, item_id: int) -> Optional[Dict]:
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                _log.error(f"API 请求失败: {res.status_code}")
                return None
        except Exception as e:
            _log.error(f"获取市场数据时发生错误: {e}")
            return None
    ...
    def format_market_data(
        self, data: Dict, item_name: str, server_name: str, hq: bool
    ) -> str:
        """
        ...
        if last_upload_time:
            last_upload_time_str = datetime.fromtimestamp(
                last_upload_time / 1000
