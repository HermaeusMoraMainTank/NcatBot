    ...
    async def handle_universalis(self, input: GroupMessage):
        ...
        if len(key) == 3 and key[0] == "搜索物品":
            ...
            if server_name in self.supported_dc:
                ...
                await self.api.post_group_msg(group_id=input.group_id, text=f"未知的服务器或大区: {server_alias}").reply()
                return
            ...
            if item_id <= 0:
                ...
                if item_id <= 0:
                    await self.api.post_group_msg(group_id=input.group_id, text=f"未找到物品: {item_name}").reply()
                    return

            # 获取市场数据
            data = self.get_data(server_name, item_id)
            if not data:
                await self.api.post_group_msg(group_id=input.group_id, text="没有找到数据喵").reply()
                return
            if not data.get("listings"):
                await self.api.post_group_msg(group_id=input.group_id, text="没有数据喵").reply()
                return

            # 格式化输出
            msg = self.format_market_data(data, item_name, server_name, hq)
            await self.api.post_group_msg(group_id=input.group_id, text=msg).reply()
