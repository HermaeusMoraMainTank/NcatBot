...
class Fortune(NcatBotPlugin):
    ...
    async def handle_fortune(self, input: GroupMessage):
        ...
        if message == "运势":
            ...
            if current_date == last_invocation_date:
                message = MessageChain(
                    [PlainText(text="你今天已经获取过运势了，请明天再来吧。")]
                )
                await self.api.qq.post_group_msg(group_id=input.group_id, rtf=message)
                return
            ...
            try:
                ...
                _log.warning(f"Error generating fortune image: {e}")
            ...
            if target_user_id in self.last_invocation_date_by_user:
                ...
                message = MessageChain(
                    [PlainText(text=f"成功重置了 {target_username} 的运势。")]
                )
                await self.api.qq.post_group_msg(group_id=input.group_id, rtf=message)
    ...
    def get_luck_info(self) -> List[str]:
        ...
        try:
            ...
            _log.warning(f"Error reading luck info: {e}")
            return ["", ""]
