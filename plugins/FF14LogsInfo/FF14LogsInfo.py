...
COMMAND_SEARCH = "搜索玩家logs"

HELP_TEXT = format_help(
    "FF14LogsInfo 玩家 Logs",
    [
        f"{COMMAND_SEARCH} <角色名> <服务器>：查询 FF Logs 排名卡片",
        "示例：搜索玩家logs 武术有栖 延夏",
    ],
)
...
class FF14LogsInfo(NcatBotPlugin):
        ...
        if os.path.exists(self.TOKEN_FILE_PATH):
            try:
                with open(self.TOKEN_FILE_PATH, "r", encoding="utf-8") as file:
                    ...
                    if self.api_token:
                        _log.info("Successfully loaded token from file")
                        return
            except Exception as e:
                _log.warning(f"Error reading token file: {e}")
    ...
    def retrieve_and_save_access_token(self):
        try:
            _log.info("Attempting to retrieve new access token...")
            ...
            if not access_token:
                _log.warning("No access token in response")
                _log.warning(f"Response content: {response.text}")
                return

            _log.info("Successfully retrieved new access token")
            self.save_access_token_to_file(response.text)
            self.api_token = access_token
        except httpx.HTTPError as e:
            _log.warning(f"HTTP error while retrieving access token: {e}")
            _log.warning(
                f"Response content: {e.response.text if hasattr(e, 'response') else 'No response content'}"
            )
        except Exception as e:
            _log.warning(f"Unexpected error while retrieving access token: {e}")
    ...
    def save_access_token_to_file(self, access_token_json: str):
        try:
            ...
            _log.info(f"Successfully saved token to {self.TOKEN_FILE_PATH}")
        except Exception as e:
            _log.warning(f"Error saving token to file: {e}")
    ...
    def get_data(self, name: str, server: str, zone_id: int) -> Optional[str]:
        ...
        if not self.api_token:
            self.retrieve_and_save_access_token()
            if not self.api_token:
                _log.warning("Failed to obtain access token")
                return None
        ...
        try:
            ...
            _log.warning(f"Error fetching data: {e}")
            return None
    ...
    def get_cached_image(self, url: str, cache_dir: str) -> PILImage.Image:
        # 从URL中提取文件名
        ...
        if os.path.exists(cache_path):
            try:
                ...
                _log.warning(f"Error loading cached image: {e}")
        ...
        try:
            ...
            _log.warning(f"Error downloading image: {e}")
            raise
    ...
    def draw_ranking_info(
        ...
        try:
            ...
            _log.warning(f"Error drawing boss icon: {e}")
        ...
        try:
            ...
            # 创建一个新的透明背景图像
            transparent_icon = PILImage.new("RGBA", job_icon.size, (0, 0, 0, 0))
            ...
            _log.warning(f"Error drawing job icon: {e}")
    ...
    def get_rank_color(self, rank: float) -> tuple:
        ...
        if (
            ...
            await self.api.qq.post_group_msg(group_id=input.group_id, text="?")
            return
        ...
        for zone in self.zones:
            ...
            if data:
                try:
                    ...
                    _log.warning(f"Error parsing data: {e}")
