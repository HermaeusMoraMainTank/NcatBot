...
### FakeAi 图片识别缓存 (`test_fakeai_vision_cache.py`)

| 规范 ID | 说明 | 验证点 |
|---------|------|--------|
| VC-01 | 缓存键生成 | QQ `file` 字段作为稳定图片身份 |
| VC-02 | 本地路径归一化 | `file:///` / 路径只保留文件名 |
| VC-03 | 无稳定 file | 返回空键，不缓存过期 URL |
| VC-04 | 保存与命中 | 同图再次读取直接返回缓存描述 |
| VC-05 | 场景隔离 | brief / detail 使用不同缓存条目 |
| VC-06 | 3 天清理 | 超过保留期的记录被删除 |
